...
public class DFDF {
    ...
    public static void comprarGasolina(Scanner gaa) {
        String[] tiposGasolina = new String[]{"Max-D(18.86)", "G-Regular(19.81)", "G-Premium(21.22)"};
        double[] preciosGasolina = new double[]{15.99, 16.79, 17.99};
        System.out.println("\nTipos de gasolina disponibles:");

        int tipoGasolina;
        for(tipoGasolina = 0; tipoGasolina < tiposGasolina.length; ++tipoGasolina) {
            System.out.println(tipoGasolina + 1 + ". " + tiposGasolina[tipoGasolina] + " - S/  por galón");
        }
        ...
        double subtotal = precioGasolina * (double)cantidadGasolina;
        double igv = subtotal * 0.18;
        double totalPagar = subtotal + igv;
        System.out.println("\n--- Detalles de la Compra ---");
        System.out.println("Tipo de gasolina: " + tiposGasolina[tipoGasolina - 1]);
        System.out.println("Precio por galón: S/ " + precioGasolina);
        System.out.println("Cantidad de galones: " + cantidadGasolina);
        System.out.println("Subtotal: S/ " + subtotal);
        System.out.println("IGV (18%): S/ " + igv);
        System.out.println("Total a pagar: S/ " + totalPagar);
        String[][] detallesPago = new String[][]{{"Ingrese el monto de pago en efectivo: ", "El monto ingresado es insuficiente. Por favor, ingrese un monto válido."}, {"Ingrese el número de la tarjeta de crédito: ", "Ingrese la fecha de vencimiento (MM/AA): ", "Ingrese el código CVV: ", "Datos de tarjeta inválidos. Pago fallido. Intente nuevamente."}};
        System.out.println("\n--- Métodos de Pago ---");
        String[] metodosPago = new String[]{"Pago en Efectivo", "Pago con Tarjeta"};

        int metodoPago;
        for(metodoPago = 0; metodoPago < metodosPago.length; ++metodoPago) {
            System.out.println(metodoPago + 1 + "- " + metodosPago[metodoPago]);
        }

        System.out.print("Seleccione el método de pago (1 o 2): ");
        metodoPago = gaa.nextInt();
        switch (metodoPago) {
            case 1:
                System.out.print(detallesPago[0][0]);
                double montoPago = gaa.nextDouble();
                if (montoPago >= totalPagar) {
                    double cambio = montoPago - totalPagar;
                    System.out.println("Pago recibido. Su cambio es de S/ " + cambio);
                } else {
                    System.out.println(detallesPago[0][1]);
                }
                break;
            case 2:
                System.out.println("\n--- Pago con Tarjeta de Crédito ---");
                gaa.nextLine();
                System.out.print(detallesPago[1][0]);
                String numeroTarjeta = gaa.nextLine();
                System.out.print(detallesPago[1][1]);
                String fechaVencimiento = gaa.nextLine();
                System.out.print(detallesPago[1][2]);
                String cvv = gaa.nextLine();
                boolean tarjetaValida = verificarTarjeta1(numeroTarjeta, fechaVencimiento, cvv, totalPagar);
                if (tarjetaValida) {
                    System.out.println("Compra exitosa. Total pagado: S/ " + totalPagar);
                } else {
                    System.out.println(detallesPago[1][3]);
                }
                break;
            default:
                System.out.println("Opción no válida");
    ...
    public static void carwash(Scanner yzz) {
        ...
        int[] preciosLavado = new int[]{20, 30};
        ...
        do {
            ...
            while(true) {
                while(true) {
                    ...
                    int tipoLavadoIndex;
                    for(tipoLavadoIndex = 0; tipoLavadoIndex < tiposLavado.length; ++tipoLavadoIndex) {
                        System.out.println(tipoLavadoIndex + 1 + ". " + tiposLavado[tipoLavadoIndex] + " - S/" + preciosLavado[tipoLavadoIndex]);
                    }

                    tipoLavadoIndex = yzz.nextInt() - 1;
                    if (tipoLavadoIndex >= 0 && tipoLavadoIndex < tiposLavado.length) {
                        int precioSinIGV = preciosLavado[tipoLavadoIndex];
                        double igv = (double)precioSinIGV * 0.18;
                        double precioConIGV = (double)precioSinIGV + igv;
                        System.out.print("Ingrese su nombre: ");
                        yzz.nextLine();
                        nombreCliente = yzz.nextLine();
                        ...
                        switch (metodoPago) {
                            case 1:
                                ...
                                int montoPago = yzz.nextInt();
                                if ((double)montoPago >= precioConIGV) {
                                    double cambio = (double)montoPago - precioConIGV;
                            ...
                            case 2:
                                ...
                                long numeroTarjeta = yzz.nextLong();
                                System.out.println("Ingrese el monto a cargar a la tarjeta:");
                                int montoCarga = yzz.nextInt();
